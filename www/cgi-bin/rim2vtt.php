...
try
{
	if(!array_key_exists("savegame", $_FILES) || $_FILES['savegame']['error'] != 0 || $_FILES['savegame']['size'] <= 0)
	...
	$process = proc_open('exec /opt/rim2vtt', array(
	...
	$stderr = json_encode(stream_get_contents($pipes[2]));
	fclose($pipes[1]);
	fclose($pipes[2]);
...
catch(Exception $e)
{
	...
	$error_json = json_encode($e->getMessage());
